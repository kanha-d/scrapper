...
def store_jobs(db,cursor,jobs_arr,comp_id,upload):
    if len(jobs_arr) > 0 :
        ...
        for job in jobs_arr :
            # print(job)
            query  += "('" +  str(job[0]) + "','" + str(job[1]) + "','"+str(comp_id)+"','"+str(job[2])+"','"+str(job[3])+"','"+str(job[3])+"','"+job[4]+"','1','active','0','4','India','5000000','100000','0','0','0','contact@jobaaj.com','No Preferences','1','1','Work from Office','0','52','40','"+str(job[6][0])+"','"+str(job[6][1])+"','20','"+created_at+"','"+expire+"','free','90','upload','"+upload+"','"+str(job[5])+"'),"
            # print(query)
            # break
