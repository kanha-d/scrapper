...
def getSkills(desc,skl):
    ...
    for i in skl.index:
        ...
        pat6 = '(' +re.escape(skl['skill'][i])
        pat7 = '- ' +re.escape(skl['skill'][i])
        pattern = re.compile(pat1 + '|' + pat2 + '|' + pat3 + '|' + pat4 + '|' + pat5)
        pattern2 = re.compile(pat1 + '|' + pat2 + '|' + pat3 + '|' + pat4 + '|' + pat5 + '|' + pat6 + '|' + pat7)

        if pattern.search(strr) or pattern2.search(title.lower()):
            my_str += skill.strip() + ','
                ...
                for job in jobs: 
                    ...
                    if url not in list1:
                        ...
                        try :
                            ...
                            # if cat=="TAX" and "technology" not in title.lower():
                            #    continue
