...
for cat_value in cat_array:
    ...
    for jobIndex in range(1, int(jobsCount)+1):
        ...
        try:
            ...
            print('Excpetion finding element')
            continue
            
    
    print(len(jobs_url))

    for ext_job_url in jobs_url: 
        list1.append(ext_job_url[1])
    ...
    print('Jobs count before Unique')
    print(len(jobs_arr))
    for job in jobs_arr:
     job_url = job[2]
    ...
    print('Jobs count after Threshold Unique')
    print(len(jobs_arr))
    THRESHOLD = 10
    best_match = \
